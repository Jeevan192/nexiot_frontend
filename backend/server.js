...
app.post('/api/events', protect, async (req, res) => {
  try {
    const { title, description, category, status, date, time, venue, capacity, image, gallery, external_link, club, icon } = req.body;
    const event_id = 'ev-' + Date.now();
    const event = await Event.create({
      event_id, title, description, category, status, date, time, venue, capacity, image, gallery, external_link,
...
app.put('/api/events/:id', protect, async (req, res) => {
  try {
    const { title, description, category, status, date, time, venue, capacity, image, gallery, external_link, icon } = req.body;
    const isObjectId = mongoose.Types.ObjectId.isValid(req.params.id);
    const query = isObjectId ? { _id: req.params.id } : { event_id: req.params.id };
    const event = await Event.findOneAndUpdate(
      query,
      { title, description, category, status, date, time, venue, capacity, image, gallery, external_link, icon },
      { new: true }
    );
